...
WorldPoints BlocksVisibleInCamera(Camera2D *camera){
  float camLeft = camera->target.x - (GetScreenWidth() / 2.0f) / camera->zoom;
  float camTop  = camera->target.y - (GetScreenHeight() / 2.0f) / camera->zoom;
  float camRight  = camera->target.x + (GetScreenWidth() / 2.0f) / camera->zoom;
  float camBottom = camera->target.y + (GetScreenHeight() / 2.0f) / camera->zoom;
